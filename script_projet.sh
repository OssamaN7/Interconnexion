...
function check_internet {
    echo -e "\e[1;36mVérification de la connexion Internet...\e[0m"
    if ping -c 1 google.com &>/dev/null; then
        echo -e "\e[1;32mConnexion Internet détectée.\e[0m"
    else
        echo -e "\e[1;31mPas de connexion Internet détectée. Veuillez vérifier votre réseau.\e[0m"
        exit 1
    fi
}

function check_docker {
    echo -e "\e[1;36mVérification de Docker...\e[0m"
    if ! command -v docker &>/dev/null; then
        echo -e "\e[1;31mDocker n'est pas installé. Voulez-vous l'installer ? (o/n)\e[0m"
        read -p "> " response
        if [[ $response == "o" ]]; then
            echo -e "\e[1;32mInstallation de Docker...\e[0m"
            sudo apt update && sudo apt install -y docker.io || { 
                echo -e "\e[1;31mÉchec de l'installation de Docker.\e[0m"; exit 1; 
            }
        else
            echo -e "\e[1;31mDocker est requis pour exécuter ce script.\e[0m"
            exit 1
        fi
    fi
    echo -e "\e[1;32mDocker est installé.\e[0m"
}

function check_gnome_terminal {
    echo -e "\e[1;36mVérification de gnome-terminal...\e[0m"
    if ! command -v gnome-terminal &>/dev/null; then
        echo -e "\e[1;31mgnome-terminal n'est pas installé. Installation en cours...\e[0m"
        sudo apt update && sudo apt install -y gnome-terminal || { 
            echo -e "\e[1;31mÉchec de l'installation de gnome-terminal.\e[0m"; exit 1; 
        }
    fi
    echo -e "\e[1;32mgnome-terminal est installé.\e[0m"
}
...
function start_system {
    ...
    for i in 1 2 3; do
        ...
        ip=$((i + 1))
        docker run -dit --privileged --name machine${i} --network Reseau_Test --ip 192.168.32.${ip} machine${i}_image >/dev/null 2>&1 || { 
    ...
    docker run -dit --privileged --name routeur_test --network Reseau_Test --ip 192.168.32.40 routeur_test_image >/dev/null 2>&1 || { 
...
function launch_machines {
    ...
    for container in $containers; do
       gnome-terminal --title="$container" -- bash -c "echo 'Machine: $container'; docker exec -it $container bash; exit" &
    done
